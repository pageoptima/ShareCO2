...
const RideRequestList = ({
    ...
    // Render when aggregated ride is avialable 
    return (
        <div className="mt-10" id="popular-routes-section">
            ...
            <div className="space-y-4">
                {rideRequests.map((rideRequest, index) => (
                    <Card
                        key={index}
                        className={`bg-gradient-to-br from-[#1A3C34] to-[#2C5046] text-white border-none shadow-xl hover:shadow-2xl transition-all overflow-hidden cursor-pointer hover:scale-[1.01] ${highlightedRequestId && rideRequest.requestIds.includes(highlightedRequestId)
                            ...
                            <div className="flex flex-col gap-2">
                                <CardTitle className="text-base sm:text-lg font-medium flex items-center justify-between">
                                    ...
                                    <Badge variant="outline" className="bg-amber-500/20 text-amber-300 border-amber-500/30 text-xs">
                                        {rideRequest.requestIds.length} {rideRequest.requestIds.length === 1 ? 'Request' : 'Requests'}
                                    </Badge>
                                </CardTitle>
                            </div>
                        </CardHeader>
                        <CardContent className="p-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
                                ...
                                <div className="flex items-center text-sm sm:col-span-2">
                                    ...
                                    <span>{rideRequest.requestIds.length} {rideRequest.requestIds.length === 1 ? 'rider' : 'riders'} waiting</span>
                                </div>
                            </div>
