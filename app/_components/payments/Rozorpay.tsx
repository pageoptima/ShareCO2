...
const verifyPayment = async (payload: void) => {
    const response = await fetch(
        '/api/payments/razorpay/varify-payment',
        {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
    ...
    if (!response.ok) {
        throw new Error(`Verify payment failed: ${response.status} ${response.statusText}`);
    }
...
const RazorpayButton = (
    ...
    const handlePayment = async () => {
        ...
        const order = await createOrderMute(
            { carbonCoin },
            {
                ...
                onSuccess: () => { console.log('Order create successfull.') },
            }
        );
    ...
    if (error) return <div>Error loading Razorpay: {error}</div>;
    if (isLoading) return <button>Loading</button>
    if (isProcessing) return <button>Processing</button>
