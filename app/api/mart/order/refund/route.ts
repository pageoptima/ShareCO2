...
import { refundExternalOrder } from '@/lib/externalOrder/externalOrderServices';
...
export async function POST(req: Request) {
    try {
        ...
        const success = await refundExternalOrder(orderId);
        ...
        logger.error( 'GET /api/mart/order/refund error:', error );
        return NextResponse.json(
            { error: 'Unable to refund order' },
            { status: 500 }
        );
