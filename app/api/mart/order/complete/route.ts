...
import { completeExternalOrder } from '@/lib/externalOrder/externalOrderServices';
...
export async function POST(req: Request) {
    try {
        ...
        // Refund the external order
        const success = await completeExternalOrder(orderId);
        ...
        logger.error( 'GET /api/mart/order/complete error:', error );
        return NextResponse.json(
            { error: 'Unable to complete order' },
            { status: 500 }
        );
